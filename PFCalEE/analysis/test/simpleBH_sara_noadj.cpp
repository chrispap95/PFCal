...
int main(int argc, char** argv){//main
    ...
    TH1F* h_rechitsumdead_Si = new TH1F("h_rechitsumdead_Si","Rechitsum dead silicon",250,100,400.);
    ...
    const unsigned nDeadSiliconCell= 69917; // nDead for
    ...
    for(int i(0);i<nsidead;i++) {
        ...
        // Here is the code that enforces that any dead cell has no more than another adjascent cell
        unsigned adj_ok = 0;
        for(auto itr=deadlistsi.begin();itr!=deadlistsi.end();itr++ ) {
            if((*itr).second-497 == cd_si) adj_ok++;
            if((*itr).second-496 == cd_si) adj_ok++;
            if((*itr).second-1 == cd_si) adj_ok++;
            if((*itr).second+1 == cd_si) adj_ok++;
            if((*itr).second+496 == cd_si) adj_ok++;
            if((*itr).second+497 == cd_si) adj_ok++;
        }
        if (adj_ok <= 1) deadlistsi.insert(std::make_pair(ld_si,cd_si));
        else --i;
    }
