...
int main(int argc, char** argv){
    ...
    **********************************/
    ...
    TH1F* h_noise = new TH1F("h_noise","noise rechits spectrum",800,0,2.);
    ...
    **********************************/
    ...
    std::cout << adj_to_dead.size() << "   " << adj_to_dead_inlay.size() << std::endl;
    ...
    **********************************/
    ...
    for (unsigned ievt(0); ievt<nEvts; ++ievt){//loop on entries (events)
        ...
        for (unsigned iH(0); iH<(*rechitvec).size(); ++iH){//loop on hits
            ...
            double dR=DeltaR(etagen,phigen-TMath::Pi()/2,leta,lphi);
            ...
            double xgen = rgen*cos(phigen-TMath::Pi()/2);
            double ygen = rgen*sin(phigen-TMath::Pi()/2);
            ...
            if(dR<coneSize && dR1<53 && layer<28) {
                ...
                if(!isScint) {
                    ...
                    for(unsigned i(0); i < nDeadSiliconCell; i++){
                    ...
                    for(unsigned i(0); i < adj_to_dead_inlay.size(); i++){
                        //In layer averaging
